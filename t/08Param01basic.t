...
BEGIN {use_ok('Pod::WSDL2::Param')}
use strict;
use warnings;

eval {
	my $a1 = new Pod::WSDL2::Param();
};

ok(defined $@, 'new dies, if it does not get a string');

eval {
	my $a1 = new Pod::WSDL2::Param('myParam $string blah blah ...');
};

ok(defined $@, 'new dies, if it does not get a string beginning with _IN, _OUT, or _INOUT');

eval {
	my $a1 = new Pod::WSDL2::Param('_IN myParam string blah blah ...');
};
